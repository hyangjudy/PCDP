...
public final class SieveActor extends Sieve {
    ...
    public int countPrimes(final int limit) {
        ...
        final SieveActorActor sieveActor = new SieveActorActor(2); // filters out multiples of 2
        finish(() -> { // sends all the subsequent numbers
            for (int i = 3; i <= limit; i += 2) {
                sieveActor.send(i);
            }
            sieveActor.send(0); // sends a zero to indicate that we're done
        });
    ...
    public static final class SieveActorActor extends Actor {
        ...
        public void process(final Object msg) {
            ...
            if (candidate <= 0) { // indicates that it needs to terminate
                if (nextActor != null) {
                    nextActor.send(msg);
                }
            } else {
                ...
                if (locallyPrime) {
                    if (numLocalPrimes < MAX_LOCAL_PRIMES) { // if it is locally prime, records it
                        localPrimes[numLocalPrimes] = candidate;
                        numLocalPrimes += 1;
                    } else if (nextActor == null) { // if the nextActor is null, creates the nextActor
